async function getWeather(place) {
  const response = await fetch(
    `https://api.weatherapi.com/v1/current.json?key=fd2c1bf3e1b142a792e190311232006&q=${place}&aqi=no`
  );
...
async function getGiph(weather) {
  const response = await fetch(
    `https://api.giphy.com/v1/gifs/translate?api_key=oa6CuH85hHRJXDvxb25l0xIXHUDjRji4&s=${weather}`
  );
